document.addEventListener('DOMContentLoaded', () => {
    // Language Selector functionality - Manual Translation Redirect
    const langToggle = document.getElementById('lang-toggle');
    const currentFlag = document.getElementById('current-flag');
    const isSpanish = window.location.pathname.includes('/es/');
    
    // Set initial flag based on current path
    if (currentFlag) {
        if (isSpanish) {
            currentFlag.src = 'https://flagcdn.com/es.svg';
            if (langToggle) langToggle.title = 'Español (Clic para cambiar a Inglés)';
        } else {
            currentFlag.src = 'https://flagcdn.com/us.svg';
            if (langToggle) langToggle.title = 'English (Click to switch to Spanish)';
        }
    }
    ...
    if (langToggle) {
        langToggle.addEventListener('click', () => {
            ...
            if (isSpanish) {
                // Currently in Spanish folder, go to root (English)
                window.location.href = '../' + filename;
            } else {
                // Currently in English root, go to Spanish folder
                // If on homepage (root path /), redirect to es/index.html
                const target = (filename === '' || filename === '/') ? 'es/index.html' : 'es/' + filename;
    ...
    if (searchInput && searchContainer) {
        ...
        searchInput.addEventListener('input', (e) => {
            ...
            const searchIndex = isSpanish ? searchIndexEs : searchIndexEn;
    ...
    const benefitsEs = [
        "Precios preferenciales en todos los productos de bienestar",
        "Acceso exclusivo a productos premium y nuevos lanzamientos",
        "Un camino guiado hacia la sanación y el bienestar",
        "Educación y apoyo continuo",
        "Una comunidad impulsada por un propósito y con ideas afines",
        "Acceso exclusivo a Legacy (nuestra oportunidad de negocio)"
    ];
    
    const benefits = isSpanish ? benefitsEs : benefitsEn;
