document.addEventListener('DOMContentLoaded', () => {
    ...
    // Check if the cookie exists and contains '/es' (Spanish)
    if (gtCookie && gtCookie.includes('/es')) {
        currentLang = 'es';
        if(currentFlag) currentFlag.src = 'https://flagcdn.com/w40/es.png';
        if(langToggle) langToggle.title = 'Español (Haga clic para cambiar)';
    } else {
        if(currentFlag) currentFlag.src = 'https://flagcdn.com/w40/us.png';
        if(langToggle) langToggle.title = 'English (Click to change)';
    }
    ...
    if (langToggle && currentFlag) {
        langToggle.addEventListener('click', () => {
            ...
            if (currentLang === 'en') {
                ...
                // Also set on root domain for live site stability
                document.cookie = `googtrans=/en/es; expires=${new Date(Date.now() + 86400000).toUTCString()}; path=/; domain=${rootDomain}`;
            } else {
                // To English - Set explicit translation to original language
                setCookie('googtrans', '/en/en', 1);
                // Also set on root domain
                document.cookie = `googtrans=/en/en; expires=${new Date(Date.now() + 86400000).toUTCString()}; path=/; domain=${rootDomain}`;
                
                // Clear any old/conflicting cookies without deleting the new ones
                try {
                    localStorage.removeItem('googtrans');
                    sessionStorage.removeItem('googtrans');
                } catch(e) {}
            }
            // Reload is the most reliable way to reset Google Translate state
            location.reload();
        });
    }
